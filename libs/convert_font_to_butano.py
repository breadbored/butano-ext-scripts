...
def generate_cpp_header(output_path: str, font_name: str, size_name: str,
    ...
    lines.append(f"constexpr s8 {font_name}_{size_name}_sprite_font_character_widths[] = {{")
...
def convert_font(font_dir: str | Path, graphics_dir: str | Path, includes_dir: str | Path,
    ...
    actual_height = max_content_height if char_height > max_content_height > 0 else char_height
    ...
    generate_cpp_header(str(hpp_path), font_name, size_name, filtered_charset, char_widths, sprite_item_name)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python convert_font_to_butano.py <font_directory> [graphics_dir] [includes_dir] [font_name]")
        print("\nExample:")
        print("  python convert_font_to_butano.py absolute_10_v1 butano-fonts/graphics butano-fonts/includes")
        sys.exit(1)
